...
describe('ClientController', () => {
  ...
  beforeEach(() => {
    ...
    (CreateClientUseCase as jest.Mock).mockImplementation(() => mockCreateClientUseCase);
    (UpdateClientUseCase as jest.Mock).mockImplementation(() => mockUpdateClientUseCase);
    (FindClientByIdUseCase as jest.Mock).mockImplementation(() => mockFindClientByIdUseCase);
    (ListClientsUseCase as jest.Mock).mockImplementation(() => mockListClientsUseCase);
  ...
  describe('create', () => {
    ...
    it('should return error when creation fails', async () => {
      mockRequest.body = { name: '', email: 'joao@email.com', phone: '11999999999' };
      mockCreateClientUseCase.execute.mockResolvedValue(left('Nome é obrigatório'));

      await clientController.create(mockRequest as Request, mockResponse as Response);

      expect(mockResponse.status).toHaveBeenCalledWith(400);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Nome é obrigatório' });
    });
  });
  ...
  describe('findById', () => {
    it('should find a client by id successfully', async () => {
      ...
      await clientController.findById(mockRequest as Request, mockResponse as Response);
    ...
    it('should return 404 when client is not found', async () => {
      ...
      mockFindClientByIdUseCase.execute.mockResolvedValue(left('Cliente não encontrado'));
  ...
  describe('findAll', () => {
    it('should list all clients successfully', async () => {
      const mockClients = [
        ...
        { id: '2', name: 'Maria Santos', email: 'maria@email.com', phone: '11888888888' }
      ] as Client[];
    ...
    it('should handle errors when listing clients', async () => {
      mockListClientsUseCase.execute.mockResolvedValue(left('Erro ao listar clientes'));

      await clientController.findAll(mockRequest as Request, mockResponse as Response);

      expect(mockResponse.status).toHaveBeenCalledWith(400);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Erro ao listar clientes' });
    });
  });
});